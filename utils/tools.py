...
def config_dataset(config):
    ...
    if config["dataset"] == "nuswide_21":
        config["data_path"] = "dataset/nuswide_21"
    if config["dataset"] in ["nuswide_21_m", "nuswide_81_m"]:
        config["data_path"] = "dataset/nuswide_81"
...
def compute_result(dataloader, net, device):
    bs, clses = [], []
    net.eval()
    for img, cls in tqdm(dataloader, ncols=60):
        clses.append(cls)
...
def validate(config, Best_mAP, test_loader, dataset_loader, net, bit, epoch, num_image):
    ...
    mAP, index = CalcTopMap(trn_binary.numpy(), tst_binary.numpy(), trn_label.numpy(), tst_label.numpy(), -1,10)
    ...
    if mAP > Best_mAP:
        ...
        Save_mat(epoch=epoch + 1, output_dim=l, datasets=config.dataset,
                 ...
                 retrieval_img=trn_binary.cpu().numpy(), save_dir='.', mode_name="CFBH_AdaQuadrupletLoss",
                 mAP=Best_mAP)
        # import os
        # save_model = "/home/admin01/桌面/CXR/NestT_hash-WITHOUTcb/model_pth/AID"
        # torch.save(net.state_dict(), os.path.join(save_model,
        #                                           "model-" + str(config['dataset']) + "-" + str(l) + "-" + str(
        #                                               Best_mAP) + str(epoch) + ".pth"))
    ...
    logger.info(f"epoch:{epoch + 1} bit:{bit} dataset:{config.dataset} MAP:{mAP} Best MAP: {Best_mAP}")
