...
def save_checkpoint(args, best_checkpoint):
    ...
    if "memo" in best_checkpoint:
        torch.save(best_checkpoint, f"{args.save_dir}/e{best_checkpoint['epoch']}_{best_checkpoint['memo']}.pkl")
    else:
...
def validate_smart(args, query_loader, dbase_loader, early_stopping, epoch, **kwargs):
    ...
    val_async = kwargs.pop("val_async", True)

    if val_async and (epoch + 1) != args.n_epochs and early_stopping.counter < early_stopping.patience - 1:
...
def mean_average_precision(qB, rB, qL, rL, topk=None):
    ...
    for i in range(num_query):
        ...
        qL = torch.from_numpy(qL) if isinstance(qL, np.ndarray) else qL
        rL = torch.from_numpy(rL) if isinstance(rL, np.ndarray) else rL
        if len(qL.shape) == 1:
